...
const PlatformLink: React.FC<{ url: string; name: string; icon: React.ReactNode; hoverColor: string }> = ({ url, name, icon, hoverColor }) => (
    ...
    >
        {icon}
        <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-[10px] font-bold text-white bg-slate-900 rounded shadow-lg whitespace-nowrap opacity-0 group-hover/link:opacity-100 transition-opacity pointer-events-none z-50">
            {name}
        </span>
    </a>
);
